...
const FinanceDashboard = () => {
    ...
    useEffect(() => {
        const fetchFinanceData = async () => {
            ...
            setError(null);
            try {
                const token = localStorage.getItem('token'); // Grab token if utilizing protection context
                const headers = token ? { 'Authorization': `Bearer ${token}` } : {};

                const summaryRes = await fetch(`/api/v1/finance/summary?month=${selectedMonth}`, { headers });
                const summaryData = await summaryRes.json();

                const statusParam = activeTab !== 'All' ? `&status=${activeTab}` : '';
                const ledgerRes = await fetch(`/api/v1/finance/ledger?month=${selectedMonth}${statusParam}`, { headers });
